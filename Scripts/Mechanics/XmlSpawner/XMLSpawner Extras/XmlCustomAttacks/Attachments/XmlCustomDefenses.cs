...
namespace Server.Engines.XmlSpawner2
{
    public class XmlCustomDefenses : XmlAttachment
    {
        ...
        public static new void Initialize()
        {
            ...
            {
                if (x is XmlCustomDefenses defenses)
                {
                    defenses.FillComboList();
                    defenses.FillSpecialsList();
                }
            }
        }
                ...
                case SpecialDefenses.ParalyzingFear:
                    {
                        ...
                        {
                            creature.BeginFlee(TimeSpan.FromSeconds(6));
                        }
        ...
        //} come back later to make sure i didnt break anything
        ...
        {
            // open the specials gump
            if (parent != null && parent is Mobile mobile && mobile.Player)
            {
                mobile.CloseGump(typeof(CustomDefenseGump));
            }
        }
        ...
        private class CustomDefenseInfoGump : Gump
        {
            ...
            public CustomDefenseInfoGump(XmlCustomDefenses a, SpecialDefense s) : base(0, 0)// note add back later if needed Mobile from
            {
            ...
            public override void OnResponse(NetState state, RelayInfo info)
            {
                ...
                for (int i = 0; i < m_attachment.Specials.Count; i++)
                {
                    ...
                    if (s != null && info.ButtonID == (int)s.DefenseID + 2000)
                    {
                        ...
                        state.Mobile.SendGump(new CustomDefenseGump(state.Mobile, m_attachment));
                        state.Mobile.SendGump(new CustomDefenseInfoGump(m_attachment, s));
                        break;
                    }
                }
            }
