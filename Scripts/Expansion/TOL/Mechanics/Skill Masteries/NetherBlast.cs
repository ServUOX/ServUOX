...
namespace Server.Spells.SkillMasteries
{
    public class NetherBlastSpell : SkillMasterySpell
    {
        ...
        public List<NetherBlastItem> Items { get; set; } = new List<NetherBlastItem>();
                        ...
                        Server.Timer.DelayCall(TimeSpan.FromMilliseconds(i * 250), () =>
                              {
                                  ...
                                  if (canFit)
                                  {
                                      var item = new NetherBlastItem(Caster, 0x37CC, loc, Caster.Map);
                                      item.ProcessDelta();
        ...
        {
            Dictionary<Mobile, NetherBlastItem> list = new Dictionary<Mobile, NetherBlastItem>();
        ...
        [DispellableField]
        public class NetherBlastItem : Item
        {
            public override bool BlocksFit => true;

            public NetherBlastItem(Mobile caster, int itemID, Point3D loc, Map map)
                : base(itemID)
            {
                Visible = false;
                Movable = false;
            ...
            public NetherBlastItem(Serial serial)
                : base(serial)
            {
            }
