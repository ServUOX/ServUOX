...
namespace Server.Items
{
    public class DisguiseKit : Item
    {
        ...
        public bool ValidateUse(Mobile from)
        {
            ...
            else if (!(from is PlayerMobile pm) || pm.NpcGuild != NpcGuild.ThievesGuild)
            {
    ...
    {
        public static Dictionary<Mobile, DisguiseKitTimer> Timers { get; } = new Dictionary<Mobile, DisguiseKitTimer>();
        ...
        public static void CreateTimer(Mobile m, TimeSpan delay)
        {
            if (m != null)
            {
                ...
                {
                    Timers[m] = new DisguiseKitTimer(m, delay);
                }
            }
        }
        ...
        public class DisguiseKitTimer : Timer
        {
            private readonly Mobile m_Player;

            public DateTime Expires { get; private set; }

            public DisguiseKitTimer(Mobile m, TimeSpan delay)
                : base(delay)
            {
                m_Player = m;
