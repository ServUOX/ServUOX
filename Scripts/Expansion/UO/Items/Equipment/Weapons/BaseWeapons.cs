    ...
    public abstract class BaseWeapon : Item, IWeapon, IFactionItem, IUsesRemaining, ICraftable, ISlayer, IDurability, ISetItem, IVvVItem, IOwnerRestricted, IResource, IArtifact, ICombatEquipment, IEngravable, IQuality
    {
        ...
        */
        ...
        public static BaseWeapon GetDamageOutput(Mobile wielder, BaseWeapon weapon, out int min, out int max)
        {
            ...
            return GetDamageOutput(wielder, weapon, out int minRaw, out int maxRaw, out min, out max);
        }
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public WeaponAnimation Animation { get => (m_Animation == (WeaponAnimation)(-1) ? Core.AOS ? AosAnimation : OldAnimation : m_Animation); set => m_Animation = value; }

        [CommandProperty(AccessLevel.GameMaster)]
        public WeaponType Type { get => (m_Type == (WeaponType)(-1) ? Core.AOS ? AosType : OldType : m_Type); set => m_Type = value; }
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public int HitSound { get => (m_HitSound == -1 ? Core.AOS ? AosHitSound : OldHitSound : m_HitSound); set => m_HitSound = value; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int MissSound { get => (m_MissSound == -1 ? Core.AOS ? AosMissSound : OldMissSound : m_MissSound); set => m_MissSound = value; }
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public int DexRequirement { get => (m_DexReq == -1 ? Core.AOS ? AosDexterityReq : OldDexterityReq : m_DexReq); set => m_DexReq = value; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int IntRequirement { get => (m_IntReq == -1 ? Core.AOS ? AosIntelligenceReq : OldIntelligenceReq : m_IntReq); set => m_IntReq = value; }
        ...
        public override void AddNameProperties(ObjectPropertyList list)
        {
            ...
            if ((fcMalus ? 1 : m_AosAttributes.SpellChanneling) != 0)
            {
