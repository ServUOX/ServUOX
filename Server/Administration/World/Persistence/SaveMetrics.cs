...
namespace Server
{
    ...
    {
        private const string PerformanceCategoryName = "ServUOX";
        private const string PerformanceCategoryDesc = "Performance counters for ServUOX";
        ...
        public SaveMetrics()
        {
            if (!PerformanceCounterCategory.Exists(PerformanceCategoryName))
            {
                ...
                if (!Core.Unix)
                {
                    ...
                    {
                        if (Core.Debug)
                            Console.WriteLine("Metrics: Metrics enabled. Performance counters creation requires ServUOX to be run as Administrator once!");
                    }
                }
                else
                {
